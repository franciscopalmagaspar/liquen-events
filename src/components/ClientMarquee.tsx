...
function Mark({ name, logo }: { name: string; logo: string }) {
  ...
  return (
    <div className="flex-shrink-0 flex items-center justify-center h-12 sm:h-14">
      {failed || !logo ? (
        <span className="text-foreground/25 text-[10px] sm:text-xs font-medium tracking-[0.22em] uppercase whitespace-nowrap">
          {name}
        </span>
      ) : (
        <Image
          ...
          width={220}
          height={88}
          className="h-full w-auto max-w-[150px] sm:max-w-[185px] object-contain opacity-55 hover:opacity-90 transition-opacity duration-300 brightness-0 invert"
...
export default function ClientMarquee() {
  return (
    <div className="relative py-6 border-y border-foreground/8 overflow-hidden">
      ...
      <div className="flex items-center gap-8 sm:gap-14 animate-marquee whitespace-nowrap">
